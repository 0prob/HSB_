...
#[derive(Debug, Clone)]
pub struct Route {
    pub pools: Vec<Address>,   // length 1 or 2 (or 3 for pseudo)
    pub tokens: Vec<Address>,  // [A,B] or [A,B,A] or [A,B,C,A]
...
impl RoutePlanner {
    ...
    fn pool_dir_price(
        ...
        let meta = self.registry.get(chain_id, &pool)?;
        let p = self.pricing.get_price(chain_id, &pool)?.price;
        if p <= 0.0 {
            return None;
        }
    ...
    pub fn build_1hop_routes(&self, chain_id: u64) -> Vec<Route> {
        let metas = self.registry.by_chain_id(chain_id);
        let mut out = Vec::new();

        for p in metas {
            if self.pricing.get_price(chain_id, &p.pool).is_none() {
                continue;
            }
            for (a, b) in [(p.token0, p.token1), (p.token1, p.token0)] {
                let pr = match self.pool_dir_price(chain_id, p.pool, a, b) {
                    Some(x) => x,
                    None => continue,
                };
                out.push(Route {
    ...
    pub fn build_2hop_cycles(&self, chain_id: u64) -> Vec<Route> {
        let metas = self.registry.by_chain_id(chain_id);
        let mut out = Vec::new();
        ...
        for p1 in &metas {
            for p2 in &metas {
                ...
                if self.pricing.get_price(chain_id, &p1.pool).is_none()
                    || self.pricing.get_price(chain_id, &p2.pool).is_none()
                {
                    continue;
                }

                for (a, b) in [(p1.token0, p1.token1), (p1.token1, p1.token0)] {
                    let p_ab = match self.pool_dir_price(chain_id, p1.pool, a, b) {
                        Some(x) => x,
                        None => continue,
                    };

                    let p_ba = match self.pool_dir_price(chain_id, p2.pool, b, a) {
                        Some(x) => x,
                        None => continue,
                    };
    ...
    pub fn build_triangular_cycles(&self, chain_id: u64) -> Vec<TriRoute> {
        let metas = self.registry.by_chain_id(chain_id);
        ...
        for (&a, edges_ab) in &adj {
            for &(b, p1) in edges_ab {
                let p_ab = match self.pool_dir_price(chain_id, p1, a, b) {
                    Some(x) => x,
                    None => continue,
                };
                ...
                for &(c, p2) in edges_bc {
                    ...
                    let p_bc = match self.pool_dir_price(chain_id, p2, b, c) {
                        Some(x) => x,
                        None => continue,
                    };
                    ...
                    for &(a2, p3) in edges_ca {
                        ...
                        let p_ca = match self.pool_dir_price(chain_id, p3, c, a) {
                            Some(x) => x,
                            None => continue,
                        };
